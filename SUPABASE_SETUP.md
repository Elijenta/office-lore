...
2. SQL Editor → run `0001_schema.sql`, then `0003_xp_cleanup_triggers.sql`, then `0004_abt_dynasty.sql`, then `0005_beer_session_editing.sql`, then `0006_standalone_beer_ratings.sql`, then `0007_beer_consumption_updated_at.sql`. `0002_seed_demo_data.sql` is optional (edit the 3 placeholder emails near the top first if you use it).
